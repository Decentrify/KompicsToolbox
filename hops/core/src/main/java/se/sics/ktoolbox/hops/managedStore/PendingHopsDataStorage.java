...
public class PendingHopsDataStorage implements Storage {
    ...
    public PendingHopsDataStorage(HDFSResource resource, long fileSize) {
        this.resource = resource;
        this.fileSize = fileSize;
        this.appendPos = HDFSHelper.length(resource);
    }
    ...
    public byte[] read(Identifier readerId, final long readPos, final int readLength, Set<Integer> bufferBlocks) {
        try {
            byte[] result = HDFSHelper.read(resource, readPos, readLength);
            return result;
    ...
    private int append(byte[] bytes) {
        return HDFSHelper.append(resource, bytes);
    }
}
