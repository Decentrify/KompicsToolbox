...
public class HDFSAppendBufferImpl implements HDFSAppendBuffer, AppendDriverI {
    ...
    public HDFSAppendBufferImpl(Config config, HDFSResource resource, String user, int appendBlockNr) {
    ...
    private void temptativeWrite(int blockNr, byte[] data) {
        if (blockNr == appendBlockNr) {
            hdfsWriters.submit(new HDFSAppendTask(resource, user, this, blockNr, data));
        }
    }
}
