...
public class HostTrafficFilter extends ChannelFilter<Msg, Boolean> {
    ...
    private Logger LOG = LoggerFactory.getLogger("Setup");
    private String logPrefix = getClass().getCanonicalName();
    ...
    private boolean sameHost(Address target) {
        if (!(target instanceof DecoratedAddress)) {
            LOG.info("{}cannot resolve filter for {} - falling back to - does not match(false)",
                    new Object[]{logPrefix, target.getClass()});
            return false;
        }
        DecoratedAddress src = (DecoratedAddress) target;
