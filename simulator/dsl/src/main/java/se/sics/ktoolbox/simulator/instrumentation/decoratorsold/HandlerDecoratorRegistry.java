...
public class HandlerDecoratorRegistry {
    ...
    public static void beforeHandler(ComponentCore comp, KompicsEvent event, Handler<?> handler) {
        for(HandlerDecorator decorator : beforeHandlerDecorators) {
            decorator.beforeHandler(comp, event, handler);
        }
    }
    
     public static void beforeHandler(ComponentCore comp, PatternExtractor event, MatchedHandler handler) {
        for(HandlerDecorator decorator : beforeHandlerDecorators) {
            decorator.beforeHandler(comp, event, handler);
        }
    }

    public static void afterHandler(ComponentCore comp, KompicsEvent event, Handler<?> handler) {
        for(HandlerDecorator decorator : afterHandlerDecorators) {
            decorator.afterHandler(comp, event, handler);
        }
    }
    
    public static void afterHandler(ComponentCore comp, PatternExtractor event, MatchedHandler handler) {
        for(HandlerDecorator decorator : afterHandlerDecorators) {
            decorator.afterHandler(comp, event, handler);
        }
    }
