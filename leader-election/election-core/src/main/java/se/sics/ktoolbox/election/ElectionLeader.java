...
//        compTracker.registerPositivePort(viewUpdate);
  ...
  Handler mockedUpdateHandler = new Handler<MockedGradientUpdate>() {
    ...
    public void handle(MockedGradientUpdate event) {
      ...
      Pair<SortedSet<LEContainer>, SortedSet<LEContainer>> lowerAndHigherViewPair = ElectionHelper.
        getHigherAndLowerViews(addressContainerMap.values(), leContainerComparator, selfLEContainer);
    ...
    public void handle(ViewUpdate viewUpdate) {
      ...
      if (viewUpdate.electionRoundId != null && inElection) {
        ...
        if (electionRoundId != null && electionRoundId.equals(viewUpdate.electionRoundId)) {
          ...
          if (electionRoundTracker.getRoundId() != null && electionRoundTracker.getRoundId().equals(
            viewUpdate.electionRoundId)) {
  ...
  private void checkIfLeader() {
    ...
    if (isConverged && higherUtilityNodes.size() == 0 && !inElection && !selfLCView.isLeaderGroupMember()) {
      if (addressContainerMap.size() < electionConfig.viewSize) {
        LOG.info("{}: I think I am leader but the view:{} less than the minimum requirement, so returning.", logPrefix,
          addressContainerMap.size());
        return;
      }
  ...
  private void startVoting() {
    ...
    Collection<KAddress> leaderGroupAddress = lcRuleSet.initiateLeadership(new LEContainer(self, selfLCView),
      addressContainerMap.values(), leaderGroupSize);

    if (leaderGroupAddress.isEmpty() || leaderGroupAddress.size() < leaderGroupSize) {
      LOG.info("{}Not asserting self as leader as the leader group size:{},  is less than required.", logPrefix,
        leaderGroupAddress.size());
      return;
    }

    Promise.Request request = new Promise.Request(msgIds.randomId(), self, selfLCView, electionRoundId);
  ...
  ClassMatchedHandler promiseResponse
    ...
    @Override
    public void handle(Promise.Response response,
      BasicContentMsg<KAddress, BasicHeader<KAddress>, Promise.Response> event) {
      ...
      if (numPromises >= electionRoundTracker.getLeaderGroupInformationSize()) {
        ...
        if (electionRoundTracker.isAccepted()) {
          ...
          LeaseCommitUpdated.Request request = new LeaseCommitUpdated.Request(msgIds.randomId(), self,
  ...
  ClassMatchedHandler leaseCommitResponseHandler
    ...
    @Override
    public void handle(LeaseCommitUpdated.Response response,
      BasicContentMsg<KAddress, BasicHeader<KAddress>, LeaseCommitUpdated.Response> event) {
      ...
      if (commitResponses >= electionRoundTracker.getLeaderGroupInformationSize()) {
        ...
        if (electionRoundTracker.isLeaseCommitAccepted()) {
          ...
          trigger(new LeaderState.ElectedAsLeader(eventIds.randomId(), electionRoundTracker.getLeaderGroupInformation()), election);
          compTracker.updateState(LeaderUpdatePacket.update(self.getId(), new ArrayList<>(electionRoundTracker.
            getLeaderGroupInformation())));
  ...
  Handler leaseTimeoutHandler = new Handler<TimeoutCollection.LeaseTimeout>() {
    ...
    public void handle(TimeoutCollection.LeaseTimeout event) {
      ...
      if (leaseTimeoutId != null && leaseTimeoutId.equals(event.getTimeoutId())) {
        ...
        Collection<KAddress> lgNodes = lcRuleSet.continueLeadership(new LEContainer(self, selfLCView),
          addressContainerMap.values(), leaderGroupSize);
        ...
        if (lgNodes.isEmpty() || lgNodes.size() < leaderGroupSize) {
          ...
          ExtensionRequest request = new ExtensionRequest(msgIds.randomId(), self, publicKey, selfLCView, roundId);
          ...
          ExtensionUpdate extensionUpdate = new ExtensionUpdate(eventIds.randomId(), new ArrayList<>(lgNodes));
