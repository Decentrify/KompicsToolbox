...
//        compTracker.registerPositivePort(viewUpdate);
  ...
  Handler mockedUpdateHandler = new Handler<MockedGradientUpdate>() {
    ...
    public void handle(MockedGradientUpdate event) {
      ...
      if (ElectionHelper.isRoundConverged(oldContainerMap.keySet(), addressContainerMap.keySet(),
        electionConfig.convergenceTest)) {
        if (!isConverged) {
      ...
      // Update the views.
      Pair<SortedSet<LEContainer>, SortedSet<LEContainer>> lowerAndHigherViewPair = ElectionHelper.
        getHigherAndLowerViews(addressContainerMap.values(), leContainerComparator, selfContainer);
   ...
   */
  ClassMatchedHandler promiseHandler
    = new ClassMatchedHandler<Promise.Request, BasicContentMsg<KAddress, BasicHeader<KAddress>, Promise.Request>>() {
    @Override
    public void handle(Promise.Request request,
      BasicContentMsg<KAddress, BasicHeader<KAddress>, Promise.Request> container) {
      ...
      if (selfLCView.isLeaderGroupMember() || inElection) {
        ...
        if (request.leaderAddress.getId().equals(self.getId())) {
          ...
          acceptCandidate = cohortsRuleSet.validate(new LEContainer(container.getContent().leaderAddress, container.
            getContent().leaderView),
  ...
  Handler awaitLeaseCommitTimeoutHandler = new Handler<TimeoutCollection.AwaitLeaseCommitTimeout>() {
    ...
    public void handle(TimeoutCollection.AwaitLeaseCommitTimeout event) {
      ...
      if (awaitLeaseCommitId != null && awaitLeaseCommitId.equals(event.getTimeoutId())) {
        ...
        trigger(new ElectionState.DisableLGMembership(eventIds.randomId(), event.electionRoundId), election);
   ...
   */
  ClassMatchedHandler commitRequestHandler
    = new ClassMatchedHandler<LeaseCommitUpdated.Request, BasicContentMsg<KAddress, BasicHeader<KAddress>, LeaseCommitUpdated.Request>>() {
    @Override
    public void handle(LeaseCommitUpdated.Request request,
      BasicContentMsg<KAddress, BasicHeader<KAddress>, LeaseCommitUpdated.Request> container) {
      ...
      if (electionRoundId == null || !electionRoundId.equals(request.electionRoundId)) {
        ...
        trigger(new ElectionState.EnableLGMembership(eventIds.randomId(), electionRoundId), election);
        trigger(new LeaderUpdate(eventIds.randomId(), request.leaderPublicKey, request.leaderAddress), election);
  ...
  private void terminateElectionInformation() {
    ...
    leaderAddress = null;
    resetElectionMetaData();
    trigger(new ElectionState.DisableLGMembership(eventIds.randomId(), null), election); // round is doesnt matter at this moment.
   ...
   * as leader.
   */
  ClassMatchedHandler extensionRequestHandler
    = new ClassMatchedHandler<ExtensionRequest, BasicContentMsg<KAddress, BasicHeader<KAddress>, ExtensionRequest>>() {
    @Override
    public void handle(ExtensionRequest leaseExtensionRequest,
      BasicContentMsg<KAddress, BasicHeader<KAddress>, ExtensionRequest> container) {
      ...
      if (selfLCView.isLeaderGroupMember()) {
        ...
        if (leaderAddress != null && !leaseExtensionRequest.leaderAddress.getId().equals(leaderAddress.getId())) {
          LOG.warn(
            "{}There might be a problem with the leader extension or a special case as I received lease extension from the node other than current leader that I already has set. ",
            logPrefix);
        }
      ...
      trigger(new LeaderUpdate(eventIds.randomId(), leaseExtensionRequest.leaderPublicKey, leaseExtensionRequest.leaderAddress), election);
