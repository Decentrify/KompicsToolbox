...
public class TreeGradientComp extends ComponentDefinition {
  ...
  public TreeGradientComp(Init init) {
    ...
    ViewConfig viewConfig = new ViewConfig(gradientConfig.viewSize, gradientConfig.softMaxTemp,
      gradientConfig.oldThreshold);
    gradientFingers = new TGParentView(overlayId, new SystemKCWrapper(config()), gradientConfig, tgradientConfig,
      logPrefix,
      init.gradientFilter);
  ...
  Handler handleCroupierSample = new Handler<CroupierSample<GradientLocalView>>() {
    ...
    public void handle(CroupierSample<GradientLocalView> sample) {
      ...
      for (AgingAdrContainer<KAddress, GradientLocalView> container : sample.publicSample.values()) {
        ...
        croupierCopy.add(new GradientContainer(container.getSource(), container.getContent().appView, age, container.
          getContent().rank));
      }
      ...
      CroupierSample publishSample = new CroupierSample(eventIds.randomId(), sample.overlayId, publicSample,
        privateSample);
  ...
  private void publishSample() {
    TGradientSample sample = new TGradientSample(eventIds.randomId(), overlayId, selfView.getContent(),
      gradientNeighbours,
